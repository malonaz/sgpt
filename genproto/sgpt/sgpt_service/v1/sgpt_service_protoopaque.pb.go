// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v6.32.1
// source: sgpt/sgpt_service/v1/sgpt_service.proto

//go:build protoopaque

package v1

import (
	_ "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	_ "github.com/malonaz/core/genproto/codegen/aip/v1"
	v1 "github.com/malonaz/sgpt/genproto/sgpt/v1"
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	fieldmaskpb "google.golang.org/protobuf/types/known/fieldmaskpb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Request message for SgptService.CreateChat.
type CreateChatRequest struct {
	state                   protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_ChatId       string                 `protobuf:"bytes,1,opt,name=chat_id,json=chatId,proto3"`
	xxx_hidden_Chat         *v1.Chat               `protobuf:"bytes,2,opt,name=chat,proto3"`
	xxx_hidden_RequestId    string                 `protobuf:"bytes,3,opt,name=request_id,json=requestId,proto3"`
	xxx_hidden_ValidateOnly bool                   `protobuf:"varint,4,opt,name=validate_only,json=validateOnly,proto3"`
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *CreateChatRequest) Reset() {
	*x = CreateChatRequest{}
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateChatRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateChatRequest) ProtoMessage() {}

func (x *CreateChatRequest) ProtoReflect() protoreflect.Message {
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *CreateChatRequest) GetChatId() string {
	if x != nil {
		return x.xxx_hidden_ChatId
	}
	return ""
}

func (x *CreateChatRequest) GetChat() *v1.Chat {
	if x != nil {
		return x.xxx_hidden_Chat
	}
	return nil
}

func (x *CreateChatRequest) GetRequestId() string {
	if x != nil {
		return x.xxx_hidden_RequestId
	}
	return ""
}

func (x *CreateChatRequest) GetValidateOnly() bool {
	if x != nil {
		return x.xxx_hidden_ValidateOnly
	}
	return false
}

func (x *CreateChatRequest) SetChatId(v string) {
	x.xxx_hidden_ChatId = v
}

func (x *CreateChatRequest) SetChat(v *v1.Chat) {
	x.xxx_hidden_Chat = v
}

func (x *CreateChatRequest) SetRequestId(v string) {
	x.xxx_hidden_RequestId = v
}

func (x *CreateChatRequest) SetValidateOnly(v bool) {
	x.xxx_hidden_ValidateOnly = v
}

func (x *CreateChatRequest) HasChat() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Chat != nil
}

func (x *CreateChatRequest) ClearChat() {
	x.xxx_hidden_Chat = nil
}

type CreateChatRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// The ID to use for the resource, which will become the final component of
	// the resource name.
	//
	// This value should be 4-63 characters, and valid characters
	// are /[a-z][0-9]-/.
	ChatId string
	// The chat to create.
	Chat *v1.Chat
	// A unique identifier for this request. Must be a UUID.
	// This request is only idempotent if a `request_id` is provided.
	RequestId string
	// If set, validate the request and preview the review, but do not actually create the resource.
	ValidateOnly bool
}

func (b0 CreateChatRequest_builder) Build() *CreateChatRequest {
	m0 := &CreateChatRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_ChatId = b.ChatId
	x.xxx_hidden_Chat = b.Chat
	x.xxx_hidden_RequestId = b.RequestId
	x.xxx_hidden_ValidateOnly = b.ValidateOnly
	return m0
}

// Request message for SgptService.GetChat.
type GetChatRequest struct {
	state           protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Name string                 `protobuf:"bytes,1,opt,name=name,proto3"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *GetChatRequest) Reset() {
	*x = GetChatRequest{}
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetChatRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetChatRequest) ProtoMessage() {}

func (x *GetChatRequest) ProtoReflect() protoreflect.Message {
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *GetChatRequest) GetName() string {
	if x != nil {
		return x.xxx_hidden_Name
	}
	return ""
}

func (x *GetChatRequest) SetName(v string) {
	x.xxx_hidden_Name = v
}

type GetChatRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// The resource name of the chat to retrieve.
	// Format: chats/{chat}
	Name string
}

func (b0 GetChatRequest_builder) Build() *GetChatRequest {
	m0 := &GetChatRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Name = b.Name
	return m0
}

// Request message for SgptService.UpdateChat.
type UpdateChatRequest struct {
	state                 protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Chat       *v1.Chat               `protobuf:"bytes,1,opt,name=chat,proto3"`
	xxx_hidden_UpdateMask *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3"`
	unknownFields         protoimpl.UnknownFields
	sizeCache             protoimpl.SizeCache
}

func (x *UpdateChatRequest) Reset() {
	*x = UpdateChatRequest{}
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateChatRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateChatRequest) ProtoMessage() {}

func (x *UpdateChatRequest) ProtoReflect() protoreflect.Message {
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *UpdateChatRequest) GetChat() *v1.Chat {
	if x != nil {
		return x.xxx_hidden_Chat
	}
	return nil
}

func (x *UpdateChatRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.xxx_hidden_UpdateMask
	}
	return nil
}

func (x *UpdateChatRequest) SetChat(v *v1.Chat) {
	x.xxx_hidden_Chat = v
}

func (x *UpdateChatRequest) SetUpdateMask(v *fieldmaskpb.FieldMask) {
	x.xxx_hidden_UpdateMask = v
}

func (x *UpdateChatRequest) HasChat() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Chat != nil
}

func (x *UpdateChatRequest) HasUpdateMask() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_UpdateMask != nil
}

func (x *UpdateChatRequest) ClearChat() {
	x.xxx_hidden_Chat = nil
}

func (x *UpdateChatRequest) ClearUpdateMask() {
	x.xxx_hidden_UpdateMask = nil
}

type UpdateChatRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// The chat to update with. The name must match or be empty.
	// The chat's `name` field is used to identify the chat to be updated.
	// Format: chats/{chat}
	Chat *v1.Chat
	// The list of fields to update.
	UpdateMask *fieldmaskpb.FieldMask
}

func (b0 UpdateChatRequest_builder) Build() *UpdateChatRequest {
	m0 := &UpdateChatRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Chat = b.Chat
	x.xxx_hidden_UpdateMask = b.UpdateMask
	return m0
}

// Request message for SgptService.DeleteChat.
type DeleteChatRequest struct {
	state                   protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Name         string                 `protobuf:"bytes,1,opt,name=name,proto3"`
	xxx_hidden_AllowMissing bool                   `protobuf:"varint,2,opt,name=allow_missing,json=allowMissing,proto3"`
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}

func (x *DeleteChatRequest) Reset() {
	*x = DeleteChatRequest{}
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteChatRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteChatRequest) ProtoMessage() {}

func (x *DeleteChatRequest) ProtoReflect() protoreflect.Message {
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *DeleteChatRequest) GetName() string {
	if x != nil {
		return x.xxx_hidden_Name
	}
	return ""
}

func (x *DeleteChatRequest) GetAllowMissing() bool {
	if x != nil {
		return x.xxx_hidden_AllowMissing
	}
	return false
}

func (x *DeleteChatRequest) SetName(v string) {
	x.xxx_hidden_Name = v
}

func (x *DeleteChatRequest) SetAllowMissing(v bool) {
	x.xxx_hidden_AllowMissing = v
}

type DeleteChatRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// The resource name of the chat to delete.
	// Format: chats/{chat}
	Name string
	// If set to true, and the book is not found, the request will succeed
	// but no action will be taken on the server
	AllowMissing bool
}

func (b0 DeleteChatRequest_builder) Build() *DeleteChatRequest {
	m0 := &DeleteChatRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Name = b.Name
	x.xxx_hidden_AllowMissing = b.AllowMissing
	return m0
}

// Request message for SgptService.ListChats.
type ListChatsRequest struct {
	state                  protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_PageSize    int32                  `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3"`
	xxx_hidden_PageToken   string                 `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3"`
	xxx_hidden_OrderBy     string                 `protobuf:"bytes,3,opt,name=order_by,json=orderBy,proto3"`
	xxx_hidden_Filter      string                 `protobuf:"bytes,4,opt,name=filter,proto3"`
	xxx_hidden_ShowDeleted bool                   `protobuf:"varint,5,opt,name=show_deleted,json=showDeleted,proto3"`
	unknownFields          protoimpl.UnknownFields
	sizeCache              protoimpl.SizeCache
}

func (x *ListChatsRequest) Reset() {
	*x = ListChatsRequest{}
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListChatsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListChatsRequest) ProtoMessage() {}

func (x *ListChatsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *ListChatsRequest) GetPageSize() int32 {
	if x != nil {
		return x.xxx_hidden_PageSize
	}
	return 0
}

func (x *ListChatsRequest) GetPageToken() string {
	if x != nil {
		return x.xxx_hidden_PageToken
	}
	return ""
}

func (x *ListChatsRequest) GetOrderBy() string {
	if x != nil {
		return x.xxx_hidden_OrderBy
	}
	return ""
}

func (x *ListChatsRequest) GetFilter() string {
	if x != nil {
		return x.xxx_hidden_Filter
	}
	return ""
}

func (x *ListChatsRequest) GetShowDeleted() bool {
	if x != nil {
		return x.xxx_hidden_ShowDeleted
	}
	return false
}

func (x *ListChatsRequest) SetPageSize(v int32) {
	x.xxx_hidden_PageSize = v
}

func (x *ListChatsRequest) SetPageToken(v string) {
	x.xxx_hidden_PageToken = v
}

func (x *ListChatsRequest) SetOrderBy(v string) {
	x.xxx_hidden_OrderBy = v
}

func (x *ListChatsRequest) SetFilter(v string) {
	x.xxx_hidden_Filter = v
}

func (x *ListChatsRequest) SetShowDeleted(v bool) {
	x.xxx_hidden_ShowDeleted = v
}

type ListChatsRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Requested page size. Server may return fewer chats than requested.
	// If unspecified, server will pick an appropriate default.
	PageSize int32
	// A token identifying a page of results the server should return.
	// This is the value of
	// [ListChatsResponse.next_page_token][chat.chat_service.v1.ListChatsResponse.next_page_token]
	// returned from the previous call to `ListChats` method.
	PageToken string
	// Can order by...
	OrderBy string
	// Can filter on id, user_id, create_time, update_time, and delete_time.
	Filter string
	// If set to true, soft deleted resources will be shown.
	ShowDeleted bool
}

func (b0 ListChatsRequest_builder) Build() *ListChatsRequest {
	m0 := &ListChatsRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_PageSize = b.PageSize
	x.xxx_hidden_PageToken = b.PageToken
	x.xxx_hidden_OrderBy = b.OrderBy
	x.xxx_hidden_Filter = b.Filter
	x.xxx_hidden_ShowDeleted = b.ShowDeleted
	return m0
}

// Response message for SgptService.ListChats.
type ListChatsResponse struct {
	state                    protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Chats         *[]*v1.Chat            `protobuf:"bytes,1,rep,name=chats,proto3"`
	xxx_hidden_NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3"`
	unknownFields            protoimpl.UnknownFields
	sizeCache                protoimpl.SizeCache
}

func (x *ListChatsResponse) Reset() {
	*x = ListChatsResponse{}
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListChatsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListChatsResponse) ProtoMessage() {}

func (x *ListChatsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *ListChatsResponse) GetChats() []*v1.Chat {
	if x != nil {
		if x.xxx_hidden_Chats != nil {
			return *x.xxx_hidden_Chats
		}
	}
	return nil
}

func (x *ListChatsResponse) GetNextPageToken() string {
	if x != nil {
		return x.xxx_hidden_NextPageToken
	}
	return ""
}

func (x *ListChatsResponse) SetChats(v []*v1.Chat) {
	x.xxx_hidden_Chats = &v
}

func (x *ListChatsResponse) SetNextPageToken(v string) {
	x.xxx_hidden_NextPageToken = v
}

type ListChatsResponse_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// The list of chats.
	Chats []*v1.Chat
	// A token to retrieve next page of results. Pass this value in the
	// [ListChatsRequest.page_token][chat.chat_service.v1.ListChatsRequest.page_token]
	// field in the subsequent call to `ListChats` method to retrieve the next
	// page of results.
	NextPageToken string
}

func (b0 ListChatsResponse_builder) Build() *ListChatsResponse {
	m0 := &ListChatsResponse{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Chats = &b.Chats
	x.xxx_hidden_NextPageToken = b.NextPageToken
	return m0
}

// Request message for SgptService.SearchChats.
type SearchChatsRequest struct {
	state                protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Filter    string                 `protobuf:"bytes,1,opt,name=filter,proto3"`
	xxx_hidden_Query     string                 `protobuf:"bytes,2,opt,name=query,proto3"`
	xxx_hidden_PageSize  int32                  `protobuf:"varint,3,opt,name=page_size,json=pageSize,proto3"`
	xxx_hidden_PageToken string                 `protobuf:"bytes,4,opt,name=page_token,json=pageToken,proto3"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *SearchChatsRequest) Reset() {
	*x = SearchChatsRequest{}
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SearchChatsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchChatsRequest) ProtoMessage() {}

func (x *SearchChatsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *SearchChatsRequest) GetFilter() string {
	if x != nil {
		return x.xxx_hidden_Filter
	}
	return ""
}

func (x *SearchChatsRequest) GetQuery() string {
	if x != nil {
		return x.xxx_hidden_Query
	}
	return ""
}

func (x *SearchChatsRequest) GetPageSize() int32 {
	if x != nil {
		return x.xxx_hidden_PageSize
	}
	return 0
}

func (x *SearchChatsRequest) GetPageToken() string {
	if x != nil {
		return x.xxx_hidden_PageToken
	}
	return ""
}

func (x *SearchChatsRequest) SetFilter(v string) {
	x.xxx_hidden_Filter = v
}

func (x *SearchChatsRequest) SetQuery(v string) {
	x.xxx_hidden_Query = v
}

func (x *SearchChatsRequest) SetPageSize(v int32) {
	x.xxx_hidden_PageSize = v
}

func (x *SearchChatsRequest) SetPageToken(v string) {
	x.xxx_hidden_PageToken = v
}

type SearchChatsRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// A filter expression.
	Filter string
	// The search query string to match against chat content.
	//
	// This performs a full-text search across all messages in each chat's transcript.
	// The search uses PostgreSQL's `plainto_tsquery` with the 'simple' dictionary,
	// which means:
	// - Multiple words are treated as an AND query (all words must be present)
	// - Common stopwords are NOT removed (due to 'simple' configuration)
	// - No stemming is applied (searches for exact word forms)
	// - Special characters and punctuation are ignored
	//
	// Results are automatically ranked by relevance, with the most relevant
	// chats appearing first.
	//
	// Examples:
	// - "refund policy" - matches chats containing both "refund" AND "policy"
	// - "order 12345" - matches chats containing both "order" AND "12345"
	// - "how to cancel" - matches chats containing all three words
	Query string
	// Requested page size. Server may return fewer chats than requested.
	// If unspecified, server will pick an appropriate default.
	PageSize int32
	// A token identifying a page of results the server should return.
	// This is the value of
	// [SearchChatsResponse.next_page_token][chat.chat_service.v1.SearchChatsResponse.next_page_token]
	// returned from the previous call to `SearchChats` method.
	PageToken string
}

func (b0 SearchChatsRequest_builder) Build() *SearchChatsRequest {
	m0 := &SearchChatsRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Filter = b.Filter
	x.xxx_hidden_Query = b.Query
	x.xxx_hidden_PageSize = b.PageSize
	x.xxx_hidden_PageToken = b.PageToken
	return m0
}

// Response message for SgptService.SearchChats.
type SearchChatsResponse struct {
	state                    protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Chats         *[]*v1.Chat            `protobuf:"bytes,1,rep,name=chats,proto3"`
	xxx_hidden_NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3"`
	unknownFields            protoimpl.UnknownFields
	sizeCache                protoimpl.SizeCache
}

func (x *SearchChatsResponse) Reset() {
	*x = SearchChatsResponse{}
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SearchChatsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchChatsResponse) ProtoMessage() {}

func (x *SearchChatsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *SearchChatsResponse) GetChats() []*v1.Chat {
	if x != nil {
		if x.xxx_hidden_Chats != nil {
			return *x.xxx_hidden_Chats
		}
	}
	return nil
}

func (x *SearchChatsResponse) GetNextPageToken() string {
	if x != nil {
		return x.xxx_hidden_NextPageToken
	}
	return ""
}

func (x *SearchChatsResponse) SetChats(v []*v1.Chat) {
	x.xxx_hidden_Chats = &v
}

func (x *SearchChatsResponse) SetNextPageToken(v string) {
	x.xxx_hidden_NextPageToken = v
}

type SearchChatsResponse_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// The list of chats matching the search query.
	Chats []*v1.Chat
	// A token to retrieve next page of results. Pass this value in the
	// [SearchChatsRequest.page_token][chat.chat_service.v1.SearchChatsRequest.page_token]
	// field in the subsequent call to `SearchChats` method to retrieve the next
	// page of results.
	NextPageToken string
}

func (b0 SearchChatsResponse_builder) Build() *SearchChatsResponse {
	m0 := &SearchChatsResponse{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Chats = &b.Chats
	x.xxx_hidden_NextPageToken = b.NextPageToken
	return m0
}

var File_sgpt_sgpt_service_v1_sgpt_service_proto protoreflect.FileDescriptor

const file_sgpt_sgpt_service_v1_sgpt_service_proto_rawDesc = "" +
	"\n" +
	"'sgpt/sgpt_service/v1/sgpt_service.proto\x12\x14sgpt.sgpt_service.v1\x1a\x1bbuf/validate/validate.proto\x1a\x1cgoogle/api/annotations.proto\x1a\x17google/api/client.proto\x1a\x19google/api/resource.proto\x1a google/protobuf/field_mask.proto\x1a malonaz/codegen/aip/v1/aip.proto\x1a\x12sgpt/v1/chat.proto\"\xcc\x01\n" +
	"\x11CreateChatRequest\x12>\n" +
	"\achat_id\x18\x01 \x01(\tB%\xbaH\"\xd8\x01\x01r\x1d\x10\x01\x18?2\x17^[a-z0-9](-?[a-z0-9])*$R\x06chatId\x12)\n" +
	"\x04chat\x18\x02 \x01(\v2\r.sgpt.v1.ChatB\x06\xbaH\x03\xc8\x01\x01R\x04chat\x12'\n" +
	"\n" +
	"request_id\x18\x03 \x01(\tB\b\xbaH\x05r\x03\xb0\x01\x01R\trequestId\x12#\n" +
	"\rvalidate_only\x18\x04 \x01(\bR\fvalidateOnly\">\n" +
	"\x0eGetChatRequest\x12,\n" +
	"\x04name\x18\x01 \x01(\tB\x18\xfaA\x0f\n" +
	"\rsgpt.com/Chat\xbaH\x03\xc8\x01\x01R\x04name\"\xe5\x01\n" +
	"\x11UpdateChatRequest\x12)\n" +
	"\x04chat\x18\x01 \x01(\v2\r.sgpt.v1.ChatB\x06\xbaH\x03\xc8\x01\x01R\x04chat\x12C\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"updateMask:`\xbaH1\x1a/\n" +
	"\x011\x12\x15chat.name must be set\x1a\x13has(this.chat.name)\xea\x9c\xc1\x03'\n" +
	"\x04tags\n" +
	"\x05files\n" +
	"\bmetadata\n" +
	"\x0emetadata.title\"f\n" +
	"\x11DeleteChatRequest\x12,\n" +
	"\x04name\x18\x01 \x01(\tB\x18\xfaA\x0f\n" +
	"\rsgpt.com/Chat\xbaH\x03\xc8\x01\x01R\x04name\x12#\n" +
	"\rallow_missing\x18\x02 \x01(\bR\fallowMissing\"\xef\x01\n" +
	"\x10ListChatsRequest\x12'\n" +
	"\tpage_size\x18\x01 \x01(\x05B\n" +
	"\xbaH\a\x1a\x05\x18\xe8\a(\x00R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x19\n" +
	"\border_by\x18\x03 \x01(\tR\aorderBy\x12\x16\n" +
	"\x06filter\x18\x04 \x01(\tR\x06filter\x12!\n" +
	"\fshow_deleted\x18\x05 \x01(\bR\vshowDeleted:=\x82\xf3-\x03\b\xc8\x01\x8a\xf3-+\n" +
	"\vcreate_time\n" +
	"\vupdate_time\x12\x0fcreate_time asc\x92\xf3-\x03\n" +
	"\x01*\"`\n" +
	"\x11ListChatsResponse\x12#\n" +
	"\x05chats\x18\x01 \x03(\v2\r.sgpt.v1.ChatR\x05chats\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"\xa2\x01\n" +
	"\x12SearchChatsRequest\x12\x16\n" +
	"\x06filter\x18\x01 \x01(\tR\x06filter\x12\x1c\n" +
	"\x05query\x18\x02 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x05query\x12'\n" +
	"\tpage_size\x18\x03 \x01(\x05B\n" +
	"\xbaH\a\x1a\x05\x18\xe8\a(\x00R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x04 \x01(\tR\tpageToken:\x0e\x82\xf3-\x03\b\xc8\x01\x92\xf3-\x03\n" +
	"\x01*\"b\n" +
	"\x13SearchChatsResponse\x12#\n" +
	"\x05chats\x18\x01 \x03(\v2\r.sgpt.v1.ChatR\x05chats\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken2\xa5\x06\n" +
	"\vSgptService\x12w\n" +
	"\n" +
	"CreateChat\x12'.sgpt.sgpt_service.v1.CreateChatRequest\x1a\r.sgpt.v1.Chat\"1\xdaA\x04chat´-\x0f\n" +
	"\rsgpt.com/Chat\x82\xd3\xe4\x93\x02\x11:\x04chat\"\t/v1/chats\x12\x91\x01\n" +
	"\n" +
	"UpdateChat\x12'.sgpt.sgpt_service.v1.UpdateChatRequest\x1a\r.sgpt.v1.Chat\"K\xdaA\x10chat,update_mask´-\x0f\n" +
	"\rsgpt.com/Chat\x82\xd3\xe4\x93\x02\x1f:\x04chat2\x17/v1/{chat.name=chats/*}\x12z\n" +
	"\n" +
	"DeleteChat\x12'.sgpt.sgpt_service.v1.DeleteChatRequest\x1a\r.sgpt.v1.Chat\"4\xdaA\x04name´-\x0f\n" +
	"\rsgpt.com/Chat\x82\xd3\xe4\x93\x02\x14*\x12/v1/{name=chats/*}\x12t\n" +
	"\aGetChat\x12$.sgpt.sgpt_service.v1.GetChatRequest\x1a\r.sgpt.v1.Chat\"4\xdaA\x04name´-\x0f\n" +
	"\rsgpt.com/Chat\x82\xd3\xe4\x93\x02\x14\x12\x12/v1/{name=chats/*}\x12\x82\x01\n" +
	"\tListChats\x12&.sgpt.sgpt_service.v1.ListChatsRequest\x1a'.sgpt.sgpt_service.v1.ListChatsResponse\"$´-\x0f\n" +
	"\rsgpt.com/Chat\x82\xd3\xe4\x93\x02\v\x12\t/v1/chats\x12\x84\x01\n" +
	"\vSearchChats\x12(.sgpt.sgpt_service.v1.SearchChatsRequest\x1a).sgpt.sgpt_service.v1.SearchChatsResponse\" \xdaA\x05query\x82\xd3\xe4\x93\x02\x12\x12\x10/v1/chats:search\x1a\v\xcaA\bsgpt.comB7Z5github.com/malonaz/sgpt/genproto/sgpt/sgpt_service/v1b\x06proto3"

var file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes = make([]protoimpl.MessageInfo, 8)
var file_sgpt_sgpt_service_v1_sgpt_service_proto_goTypes = []any{
	(*CreateChatRequest)(nil),     // 0: sgpt.sgpt_service.v1.CreateChatRequest
	(*GetChatRequest)(nil),        // 1: sgpt.sgpt_service.v1.GetChatRequest
	(*UpdateChatRequest)(nil),     // 2: sgpt.sgpt_service.v1.UpdateChatRequest
	(*DeleteChatRequest)(nil),     // 3: sgpt.sgpt_service.v1.DeleteChatRequest
	(*ListChatsRequest)(nil),      // 4: sgpt.sgpt_service.v1.ListChatsRequest
	(*ListChatsResponse)(nil),     // 5: sgpt.sgpt_service.v1.ListChatsResponse
	(*SearchChatsRequest)(nil),    // 6: sgpt.sgpt_service.v1.SearchChatsRequest
	(*SearchChatsResponse)(nil),   // 7: sgpt.sgpt_service.v1.SearchChatsResponse
	(*v1.Chat)(nil),               // 8: sgpt.v1.Chat
	(*fieldmaskpb.FieldMask)(nil), // 9: google.protobuf.FieldMask
}
var file_sgpt_sgpt_service_v1_sgpt_service_proto_depIdxs = []int32{
	8,  // 0: sgpt.sgpt_service.v1.CreateChatRequest.chat:type_name -> sgpt.v1.Chat
	8,  // 1: sgpt.sgpt_service.v1.UpdateChatRequest.chat:type_name -> sgpt.v1.Chat
	9,  // 2: sgpt.sgpt_service.v1.UpdateChatRequest.update_mask:type_name -> google.protobuf.FieldMask
	8,  // 3: sgpt.sgpt_service.v1.ListChatsResponse.chats:type_name -> sgpt.v1.Chat
	8,  // 4: sgpt.sgpt_service.v1.SearchChatsResponse.chats:type_name -> sgpt.v1.Chat
	0,  // 5: sgpt.sgpt_service.v1.SgptService.CreateChat:input_type -> sgpt.sgpt_service.v1.CreateChatRequest
	2,  // 6: sgpt.sgpt_service.v1.SgptService.UpdateChat:input_type -> sgpt.sgpt_service.v1.UpdateChatRequest
	3,  // 7: sgpt.sgpt_service.v1.SgptService.DeleteChat:input_type -> sgpt.sgpt_service.v1.DeleteChatRequest
	1,  // 8: sgpt.sgpt_service.v1.SgptService.GetChat:input_type -> sgpt.sgpt_service.v1.GetChatRequest
	4,  // 9: sgpt.sgpt_service.v1.SgptService.ListChats:input_type -> sgpt.sgpt_service.v1.ListChatsRequest
	6,  // 10: sgpt.sgpt_service.v1.SgptService.SearchChats:input_type -> sgpt.sgpt_service.v1.SearchChatsRequest
	8,  // 11: sgpt.sgpt_service.v1.SgptService.CreateChat:output_type -> sgpt.v1.Chat
	8,  // 12: sgpt.sgpt_service.v1.SgptService.UpdateChat:output_type -> sgpt.v1.Chat
	8,  // 13: sgpt.sgpt_service.v1.SgptService.DeleteChat:output_type -> sgpt.v1.Chat
	8,  // 14: sgpt.sgpt_service.v1.SgptService.GetChat:output_type -> sgpt.v1.Chat
	5,  // 15: sgpt.sgpt_service.v1.SgptService.ListChats:output_type -> sgpt.sgpt_service.v1.ListChatsResponse
	7,  // 16: sgpt.sgpt_service.v1.SgptService.SearchChats:output_type -> sgpt.sgpt_service.v1.SearchChatsResponse
	11, // [11:17] is the sub-list for method output_type
	5,  // [5:11] is the sub-list for method input_type
	5,  // [5:5] is the sub-list for extension type_name
	5,  // [5:5] is the sub-list for extension extendee
	0,  // [0:5] is the sub-list for field type_name
}

func init() { file_sgpt_sgpt_service_v1_sgpt_service_proto_init() }
func file_sgpt_sgpt_service_v1_sgpt_service_proto_init() {
	if File_sgpt_sgpt_service_v1_sgpt_service_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_sgpt_sgpt_service_v1_sgpt_service_proto_rawDesc), len(file_sgpt_sgpt_service_v1_sgpt_service_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   8,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_sgpt_sgpt_service_v1_sgpt_service_proto_goTypes,
		DependencyIndexes: file_sgpt_sgpt_service_v1_sgpt_service_proto_depIdxs,
		MessageInfos:      file_sgpt_sgpt_service_v1_sgpt_service_proto_msgTypes,
	}.Build()
	File_sgpt_sgpt_service_v1_sgpt_service_proto = out.File
	file_sgpt_sgpt_service_v1_sgpt_service_proto_goTypes = nil
	file_sgpt_sgpt_service_v1_sgpt_service_proto_depIdxs = nil
}
